...
try:
    ...
    from services.voice_factory import VoiceFactory
...
def init_services():
    ...
    try:
        # 初始化语音识别服务（使用工厂模式，自动选择可用的服务）
        voice_service = VoiceFactory.create_voice_service('auto')
        if isinstance(voice_service, BaiduVoiceService):
            logger.info("✓ 百度语音识别服务初始化成功")
        elif isinstance(voice_service, LocalSTTService):
            logger.info("✓ 本地语音识别服务初始化成功")
        else:
            logger.info("✓ 语音识别服务初始化成功")
        
        # 初始化语音合成服务（使用工厂模式，自动选择可用的服务）
        tts_service = TTSFactory.create_tts_service('auto')
        if isinstance(tts_service, BaiduTTSService):
            logger.info("✓ 百度语音合成服务初始化成功")
        elif isinstance(tts_service, LocalTTSService):
            logger.info("✓ 本地语音合成服务初始化成功")
        else:
